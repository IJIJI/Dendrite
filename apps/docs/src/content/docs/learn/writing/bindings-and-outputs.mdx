...
import { forwardExample, unusedExample } from "../../../../examples";
...
`29.95{:den}`. They cannot ask for `subtotal{:den}`: that is working, not a result.
...
<Live
  client:only="react"
  fails
  source={forwardExample.source}
  ports={forwardExample.ports}
/>
...
A **cycle** is a different matter and always an error. If `a{:den}` reads `b{:den}` and `b{:den}`
reads `a{:den}`, neither has a value in any order, and you get a `binding_cycle{:den}` before
...
<Live
  client:only="react"
  warns
  source={unusedExample.source}
  ports={unusedExample.ports}
/>

`unused{:den}` gets an `unused_binding{:den}` warning and is pruned. Nothing breaks; you are
just told, because an unreachable binding is usually a typo or a leftover. Read it in
`answer{:den}`, or delete its line, and the warning goes.
