...
`isCompatible` walks the `extends` chain upward (a subtype is usable where its supertype is expected), plus array covariance and function variance. Always called as a function (never inlined) so it stays the single extension point. (No type *declares* `extends` in core yet — see backlog.md for the Beacon precondition.)
...
A host attaches a zod schema wherever the declaration is CODE: on the language, or on a layer it rebuilds each boot (a capability). The one place it cannot go is the layer an instance PERSISTS, because that is saved as JSON and a schema does not survive: converting one keeps enums and bounds but drops a `.refine` predicate silently (verified, zod 4.4.3), so a saved "even number" would come back accepting odd ones. `createInstance` refuses a persisted layer that declares one, next to the one-persisted-layer rule, so the mistake surfaces at construction rather than on the next reload; `isPorts` refuses one on the way in, which is the untrusted-payload case types cannot catch. A persisted type that needs validating inherits it through `extends` from a language type. Nothing in core calls a schema yet — the boundary check is in `backlog.md`.
