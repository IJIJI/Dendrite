...
import {
  descriptor, program, filteringProgram, honorsProgram,
...
for (const s of scenarios) {
  ...
  const { ms: runMs }     = time(() => run(program, descriptor, { values: s.values, threshold: s.threshold }))
  const { ms: runnerMs }  = time(() => runner.run(changes))
  const { ms: runtimeMs } = time(() => runtime.updateInputs(changes))

  rows.push({ label: s.label, count: s.values.length, changed: changesFrom(prev, s), runMs, runnerMs, runtimeMs })
  prev = s
}
