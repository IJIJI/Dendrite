...
| `session/editor.ts`            | `createEditor` - the lifecycle Facade over a connection: CodeMirror view, debounced recompile, lint, `onChange`, `dispose`                                                                                                                                                  |
...
| `session/permalink.ts`         | document ↔ URL payload (deflate + base64url, native streams)                                                                                                                                                                                                                |
| `session/carry-value.ts`       | an input's value carried across a rename or an undo once the change is published                                                                                                                                                                                            |
| `code/tokens.ts`, `code/cm.ts` | lexer-driven highlighting and the editor chrome theme; `cm.ts` + `session/editor.ts` are the only CodeMirror-aware modules                                                                                                                                                  |
| `code/diagnostic.ts`           | `positionOf` - the one adapter from a core `SourceRef` to a line and column                                                                                                                                                                                                 |
...
| `react/`                       | the compound components above - the only place React is allowed: `layouts/` (the presets and the `Row` / `Column` primitives), `panes/` (Inputs, Outputs, Diagnostics and what they share), `chrome/` (TopBar, Canvas, brand, icons); the provider and its hooks at the top |
